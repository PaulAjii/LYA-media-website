<template>
    <main class="min-h-screen text-black pb-20 pt-40 bg-gray-50 px-8">
        <header class="flex flex-col items-center justify-center -mt-12">
            ...
            <h2 class="text-2xl font-oswald font-bold md:text-3xl">WORSHIP SESSIONS</h2>

            <p class="my-4 text-md font-montserrat md:text-lg">The Emissaries Praise and Worship sessions</p>
        ...
        <section class="mt-12 font-montserrat">
            ...
            <div v-else-if="store.worshipSessions && store.worshipSessions.length > 0"
                class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                <NuxtLink v-for="ws in store.worshipSessions" :key="ws.id" :to="`/worship-sessions/${ws.id}`"
                    ...
                    <div class="w-full h-36 bg-white/80 rounded-lg mb-3 overflow-hidden">
                        <img v-if="ws.thumbnailURL" :src="ws.thumbnailURL" :alt="ws.audioURL"
                            class="w-full h-full object-cover" />
                        <div v-else class="w-full h-full flex items-center justify-center text-gray-500 text-sm">
                            <PhFolder size="150" weight="thin" />
                        </div>
                    </div>
                    <h3 class="font-semibold text-gray-800 text-sm truncate">Worship - {{ formatDate(ws.date) }}</h3>
                    <p class="text-gray-400 text-xs mt-1">Uploaded on: {{ formatDate(ws.createdAt) }}</p>
                </NuxtLink>
            </div>
...
const { fetchWorshipSessions } = useWorshipSessionService()
